...
function Daily_Wear_jewelery() {
  ...
  useEffect(() => {
    fetch(
      'http://localhost:8000/api/products/daily%20wear/Dailywear%20Jewellery'
    )
      .then((response) => {
        if (!response.ok) {
      ...
      .then((data) => {
        ...
        setJewelryData(Array.isArray(data) ? data : data.products || [])
        setLoading(false)
      })
      .catch((error) => {
  ...
  return (
    <>
      <h1 className="text-black lg:text-2xl font-bold p-2 lg:ml-8 my-3 md:text-left text-center">
        Daily Wear Jewellery ({jewelryData.length})
      </h1>
      {jewelryData.length === 0 ? (
        <p className="text-center text-gray-500">No products available</p>
      ) : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {jewelryData.map((info, index) => (
            <ProductCard
              key={index}
