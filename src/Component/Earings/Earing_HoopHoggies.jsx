...
function Earing_HoopHoggies() {
  const [earringsData, setEarringsData] = useState([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)

  useEffect(() => {
    fetch('http://localhost:8000/api/products/Earrings/Hoop%20&%20Huggies')
      .then((response) => {
        if (!response.ok) {
  ...
  return (
    <>
      <h1 className="text-black lg:text-2xl font-bold p-2 lg:ml-8 my-3 md:text-left text-center">
        Hoop Huggies ({earringsData.length})
      </h1>
...
export default Earing_HoopHoggies
