...
function Gold_Engagement() {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/products/Gold/Engagement')
      .then((response) => {
        if (!response.ok) {
  ...
  return (
    <>
      <h1 className="text-black lg:text-2xl font-bold p-2 lg:ml-8 my-3 md:text-left text-center">
        Gold Engagement ({goldData.length})
      </h1>
<<<<<<< HEAD
      ...
      {goldData.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {goldData.map((info, index) => (
            <ProductCard
              key={index}
              titleimg={info.imageFile1 || 'default-image.jpg'} // ✅ Prevents broken images
